...
public class BlService {
    ...
    public void updatePhim_deleteBl(String maphim,Binhluan bl){
        Session se=factory.openSession();
        Transaction t=se.beginTransaction();
        try {
            Phim p=(Phim) se.get(Phim.class, bl.getPhim().getMaphim());
            double newDiem=(double)(p.getDiem()*p.getLuotbl()-bl.getDiem())/(p.getLuotbl()-1);
            p.setDiem(newDiem);
            p.setLuotbl(p.getLuotbl()-1);
            se.update(p);
            t.commit();
        } catch (Exception e) {
            t.rollback();
